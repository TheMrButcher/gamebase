...
public:
    void load()
    {
        ...
        {
            auto newBall = i < 9
                ? deserialize<ObjectConstruct>("pool\\Ball.json")
                : deserialize<ObjectConstruct>("pool\\StripedBall.json");
            newBall->getChild<StaticTextureRect>("#ball")->setColor(ballColors[i]);
        ...
        filledLabels[FirstPlayer] = design->getChild<StaticLabel>("#filled1");
        filledLabels[SecondPlayer] = design->getChild<StaticLabel>("#filled2");

        stripedLabels[FirstPlayer] = design->getChild<StaticLabel>("#striped1");
        stripedLabels[SecondPlayer] = design->getChild<StaticLabel>("#striped2");

        design->getChild<StaticLabel>("#score1")->setVisible(false);
        design->getChild<StaticLabel>("#score2")->setVisible(false);
    ...
    void restart()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            filledLabels[i]->setVisible(false);
            stripedLabels[i]->setVisible(false);
            score[i] = 0;
        }
                ...
                for (int j = 0; j < 6; ++j)
                {
                    ...
                    if (distance.length() < holeRadius)
                    {
                        ...
                        if (i == 8)
                        {
                            ...
                            {
                                score[otherPlayer] = 8;
                                winLabels[otherPlayer]->setVisible(true);
                            }
                        }
                        else if (i == 0)
                        {
                        ...
                        else
                        {
                            ...
                            if (score[0] == 0 && score[1] == 0)
                            {
                                if (curPlayer == FirstPlayer)
                                    isFirstPlayerFilled = isFilled;
                                else
                                    isFirstPlayerFilled = !isFilled;
                                if (isFirstPlayerFilled)
                                {
                                    filledLabels[FirstPlayer]->setVisible(true);
                                    stripedLabels[SecondPlayer]->setVisible(true);
                                }
                                else
                                {
                                    filledLabels[SecondPlayer]->setVisible(true);
                                    stripedLabels[FirstPlayer]->setVisible(true);
                                }
                                score[curPlayer]++;
                                if (!fault)
                                    changePlayer = false;
                            ...
                            {
                                bool isGoodForFirst = isFilled == isFirstPlayerFilled;
                                if ((curPlayer == FirstPlayer) == isGoodForFirst)
                                {
                                    score[curPlayer]++;
                                    if (!fault)
                                        changePlayer = false;
    ...
    StaticLabel* filledLabels[2];
    StaticLabel* stripedLabels[2];
    bool isFirstPlayerFilled;
    Timer timer;
    bool fault;
};
